...
const modulesPlugins = ['babel-plugin-transform-es2015-modules-amd'].map((name) => require(name));
...
function babelCompile(forceCompile, componentUrl) {
    return transform_middleware_1.transformResponse({
        ...
        transform(request, response, body) {
            ...
            const cached = exports.babelCompileCache.get(optionsLeader + body);
            if (cached !== undefined) {
                return cached;
            }
            let transformed;
            ...
            exports.babelCompileCache.set(optionsLeader + body, transformed);
            return transformed;
        },
    });
}
exports.babelCompile = babelCompile;
function compileHtml(source, location, componentUrl, options) {
    ...
    for (const scriptTag of dom5.queryAll(document, isJsScriptNode)) {
        ...
        if (transformingModule && !requireScriptTag) {
            ...
            // present.
            const fragment = parse5.parseFragment(`<script src="/${componentUrl}/requirejs/require.js"></script>\n`);
        ...
        if (transformingModule && !isInline) {
            ...
            dom5.replace(scriptTag, parse5.parseFragment(`<script>require(["${src}"]);</script>\n`));
        }
        else if (isInline) {
    ...
    if (wctScriptTag && requireScriptTag) {
        ...
        dom5.insertBefore(wctScriptTag.parentNode, wctScriptTag, parse5.parseFragment(`
<script>
...
`));
        ...
        dom5.insertAfter(requireScriptTag.parentNode, requireScriptTag, parse5.parseFragment(`
<script>
  // Injected by Polyserve.
  (function() {
...
function compileScript(source, options) {
    const plugins = [];
    if (options.transformES2015) {
        plugins.push(...es2015Plugins);
    }
